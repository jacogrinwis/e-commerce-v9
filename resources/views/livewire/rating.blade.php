{{-- <div class="flex items-center space-x-1">
    <div class="flex items-center space-x-1">
        @for ($i = 1; $i <= $maxRating; $i++)
            <button
                @if (!$readonly) wire:click="setRating({{ $i - 0.5 }})"
                wire:mouseenter="setRating({{ $i - 0.5 }})" @endif
                class="{{ $readonly ? 'cursor-default' : 'cursor-pointer' }} transform text-2xl transition hover:scale-110 focus:outline-none"
            >
                @if ($i <= $rating)
                    <!-- Volle ster -->
                    <svg
